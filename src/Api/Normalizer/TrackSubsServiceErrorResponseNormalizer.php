...
class TrackSubsServiceErrorResponseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \ShipStream\Ups\Api\Model\TrackSubsServiceErrorResponse::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \ShipStream\Ups\Api\Model\TrackSubsServiceErrorResponse::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('response', $data)) {
            $object->setResponse($this->denormalizer->denormalize($data['response'], \ShipStream\Ups\Api\Model\ErrorResponseWrapper::class, 'json', $context));
            unset($data['response']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('invalidTrackingNumbers') && null !== $data->getInvalidTrackingNumbers()) {
            $values = [];
            foreach ($data->getInvalidTrackingNumbers() as $value) {
                $values[] = $value;
            }
            $dataArray['invalidTrackingNumbers'] = $values;
        }
        foreach ($data as $key => $value_1) {
            if (preg_match('/.*/', (string) $key)) {
    ...
    {
        return [\ShipStream\Ups\Api\Model\TrackSubsServiceErrorResponse::class => false];
    }
}